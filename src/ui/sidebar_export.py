...
def render_exportar():
    ...
    if st.button(":material/file_download: Exportar conversaci\u00f3n",
...
def _show_export_dialog():
    ...
    with c1:
        ...
        if d:
            st.download_button("\u2b07 Descargar JSON", data=d, file_name=d.name,
                               mime="application/json", use_container_width=True,
                               key="dl_json", type="secondary")
    with c2:
        d = mgr.export_chat_bytes(sid, tid, "markdown")
        if d:
            st.download_button("\u2b07 Descargar Markdown", data=d, file_name=d.name,
                               mime="text/markdown", use_container_width=True,
                               key="dl_md", type="secondary")
